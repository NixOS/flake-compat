let inherit (builtins) mapAttrs; in rec {
  fetchTree =
  ...
  resolveNodes = { nodes, root, rootSrc }: let allNodes = mapAttrs (
    key: node:
    let
      isRelative = node.locked.type or null == "path" && builtins.substring 0 1 node.locked.path != "/";

      parentNode = allNodes.${getInputByPath root node.parent};
      ...
      subdir = if key == root then "" else node.locked.dir or "";
      ...
      getInputByPath =
        ...
        if path == [ ] then
          ...
          getInputByPath
            ...
            (resolveInput nodes.${nodeName}.inputs.${builtins.head path})
            (builtins.tail path);
